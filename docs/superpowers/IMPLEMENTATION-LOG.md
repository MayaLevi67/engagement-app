...
**Verification:** lint (`--max-warnings 0`), typecheck, 152 unit + 11 e2e — all green. 11/11 acceptance criteria.
...
- **Admin non-admin-gate tests only transitive for 13 of 14 Phase 4 admin exports** (P3): only `createConcept` has a direct FORBIDDEN test; `updateConcept`/`deleteConcept`/`setConceptActive`/`setConceptPremium`/`reorderConcept` and all element/image mutations share the same `requireAdmin()` helper (present + reviewed in code) but aren't individually asserted. A parametrized non-admin loop over all 14 exports would cheaply harden this (same finding carried from Phase 3's template actions).
